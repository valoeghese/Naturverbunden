...
package valoeghese.naturverbunden.util.terrain.cache;

import java.util.Arrays;

public class LossyCache<T> implements GridOperator<T> {
	public LossyCache(int size, GridOperator<T> operator) {
	...
	private final GridOperator<T> operator;
	...
	public T get(int x, int y) {
		try {
			...
			if (this.positions[loc] != pos) {
				...
				return (T) (this.data[loc] = this.operator.get(x, y));
			} else {
				return (T) this.data[loc];
			}
		} catch (Exception e) {
			e.printStackTrace();
