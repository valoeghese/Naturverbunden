...
public class ImplRegistryEvent {
	static class Block implements RegistryEvent<schluessel.block.Block> {
		...
		public void register(String id, schluessel.block.Block object) {
			...
			Identifier identifier = context.locationOf(id);
			...
			// Add the item model
			ImplSchluesselARRP.RESOURCE_PACK.addModel(block.computeItemModel(identifier), this.context.locationOf("item/" + id));
