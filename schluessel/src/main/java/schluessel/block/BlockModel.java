...
public interface BlockModel {
	...
	ModelFunction CUBE_ALL_MODEL = ids -> {
		ResourceLocation id = ids.apply("", "block");
	...
	ModelFunction TALL_CROSS_MODEL = ids -> {
		ResourceLocation bottomModelId = ids.apply("", "block");
		ResourceLocation topModelId = ids.apply("top", "block");
	...
	static ModelFunction slabModel(String baseId) {
		return ids -> {
			ResourceLocation lowerSlabResourceLocation = ids.apply("", "block");
			ResourceLocation topSlabResourceLocation = ids.apply("top", "block");
			ResourceLocation baseModelResourceLocation = ids.apply("block/" + baseId);

			return ImmutableMap.of(
					// bottom model
					lowerSlabResourceLocation,
					JModel.model()
					.parent("block/slab")
					.textures(JModel.textures()
							.var("bottom", baseModelResourceLocation.toString())
							.var("top", baseModelResourceLocation.toString())
							.var("side", baseModelResourceLocation.toString())
							),
					// top model
					topSlabResourceLocation,
					JModel.model()
	...
	@FunctionalInterface
	interface IdFunction extends BiFunction<String, String, ResourceLocation> {
		/**
		 * Retrieves an ResourceLocation with either: the mod id and the "subPath" (if category is null), or: the mod id, the category, the id, and the subpath
		 */
		ResourceLocation apply(String subPath, @Nullable String category);

		default ResourceLocation apply(String path) {
	...
	interface StateFunction {
		JState createModel(ResourceLocation id, JBlockModel[] models);
	}

	@FunctionalInterface
	interface ModelFunction {
		Map<ResourceLocation, JModel> createModels(IdFunction ids);
	}

	@FunctionalInterface
	public interface AdditionalModelFunction {
		List<JBlockModel> createModels(IdFunction ids);
	}
