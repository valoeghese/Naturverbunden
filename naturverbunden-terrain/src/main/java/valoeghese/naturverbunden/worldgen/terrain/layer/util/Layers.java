...
import valoeghese.naturverbunden.worldgen.terrain.layer.RemoveLikeWaaaaayyyyTooMuchOcean;
...
public class Layers {
	...
	private static <T extends LayerSampler, C extends LayerSampleContext<T>> LayerFactory<T> build(LongFunction<C> cp) {
		...
		layer = RemoveLikeWaaaaayyyyTooMuchOcean.INSTANCE.create(cp.apply(51L), layer);
